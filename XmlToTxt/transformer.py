...
class Transformer(object):
    ...
    def write_to_txt(self, annotations, classes):
        for annotation in annotations:
            output_path = os.path.join(self.out_dir, self.YoloV5_formats(annotation.filename))
            if not os.path.exists(os.path.dirname(output_path)):
                os.makedirs(os.path.dirname(output_path))
            with open(output_path, "w+") as f:
                f.write(self.to_Yolov5_format(annotation, classes))

    def to_Yolov5_format(self, annotation, classes):
        result = []
